...
if not st.session_state["logged_in"]:
    ...
    st.write("Welcome recruit. You must link your Google account to access the guild's command center.")
    
    # Simulating a clean Google account input portal
    email_input = st.text_input("Enter your Google Email address:", placeholder="yourname@gmail.com")
    
    if st.button("🔴 Continue with Google"):
        if "@gmail.com" in email_input or "@" in email_input:
            st.session_state["logged_in"] = True
            st.session_state["google_email"] = email_input
            st.rerun()
        else:
            st.error("Please enter a valid Google account email address to authenticate.")
...
# --- GUILD LEADER DASHBOARD & SECTIONS ---
    ...
    st.write(f"Authorized via: `{st.session_state['google_email']}`")
    ...
    with portal_tabs[1]:
        ...
        # Displaying membership lists gathered from your screenshots
        members_list = [
            "♱ﾠDamzy🪶", "IME々DAVE★", "♱ YENG", "♱ KUROTSUI", "♱ CUBA",
            "B₂KMUBBy🪶", "GS JOKERツ", "🪶 HAKUTSUK", "♱ \"SONXC\"", "♱ SHADOW",
            "d🪶a🪶d🪶y🪶!", "I'm Grampx☠️", "♱ SHEGZY", "LEVI™", "♱ GHOST",
            "T WING", "♱ ITACHI", "♱ SLIME", "♱LĀŚTBØRŃ", ">ONE ISAGI¿",
            "♱ SMART", "♱ MONSTER", "jyrukilla☠️☠️", "♱ BANDIT", "♱ MORGAN",
            "乂MR▪sʜᴀdᴏu", "EvIL🪶★Tøxîç", "♱ CHARLIE", "♱ MARCELO", "♱ DRAGON•"
        ]
